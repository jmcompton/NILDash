...
app.post('/api/ai/player-lookup', requireAuth, async (req, res) => {
  ...
  const prompt = 'Search for current 2025-26 stats and social media for college athlete: ' + name + (school ? ' at ' + school : '') + (sport ? ' (' + sport + ')' : '') + '. Search their Instagram and TikTok handles for actual follower counts. Search ESPN, 247Sports, or school athletic sites for real stats. Return ONLY this JSON with real verified data (no estimates - leave field empty string if unknown): {"found":true,"name":"full legal name","school":"official school name","sport":"sport","position":"position abbrev","year":"Freshman or Sophomore or Junior or Senior or Grad Transfer","stats":"real 2025-26 season stats - PPG/RPG/APG for basketball, yards/TDs for football, ERA/AVG for baseball etc","height":"height","weight":"weight","hometown":"city state","instagram":0,"tiktok":0,"engagement":0,"schoolTier":"p4-top10 or p4-mid or p4-lower or mid-top or mid-lower","notes":"verified bio including awards, rankings, draft projection if any"}. Return ONLY JSON.';
  try {
    const raw = await ai.oneShotWithSearch(prompt, 'You are a sports researcher. Search the web for real verified athlete data. Never estimate or fabricate stats. If you cannot verify a number, use 0 for numeric fields or empty string for text. Return only valid JSON.');
