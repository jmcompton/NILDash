...
async function init() {
  ...
  // Who answered: 'anthropic' (every row before the column) or 'deepseek'
  // (services/deepseek). The breakdown groups by it.
  await pool.query(`ALTER TABLE ai_call_ledger ADD COLUMN IF NOT EXISTS provider TEXT NOT NULL DEFAULT 'anthropic'`).catch(() => {});
