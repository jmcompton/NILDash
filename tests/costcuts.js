...
async function main() {
  ...
  ok('the pitch writer was not touched: no deepseek, Sonnet on both writer sites', !/deepseek/i.test(src('server/services/pitchWriter.js')) && (src('server/jobs/outreachQueue.js').match(/site: 'writer'[\s\S]{0,300}?ai\.oneShot\(p2, sys, mt, ai\.MODEL_GEN(?:, \{ prose: true \})?\)/g) || []).length === 2);
