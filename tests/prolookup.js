...
(async () => {
  ...
  ok('  kept on the team and position the model knows, and the trace notes Wikipedia corroborated them', nix.trace.some((t) => /web candidate "Bo Nix" kept: team and position from model knowledge \(public figure\); corroborated by https:\/\/en\.wikipedia\.org\/wiki\/Bo_Nix/.test(t)), nix.trace);
  ...
  ok('a web candidate with no team or no position is dropped, and the trace says so', made.found === false && made.trace.some((t) => /web candidate "Made Upson" dropped: no team or no position/.test(t)), made.trace);
  ...
  ok('the hit-rate script exists with ten NFL, ten NBA and ten MLB players and reports the athlete, stats, Instagram handle and follower count rates per league', (() => { const s = src('scripts/lookup-pro-hitrate.js'); return /NFL: \[/.test(s) && /NBA: \[/.test(s) && /MLB: \[/.test(s) && ['Bo Nix', 'Patrick Mahomes', 'Nikola Jokic', 'Stephen Curry', 'Aaron Judge', 'Shohei Ohtani'].every((n) => s.includes(n)) && (s.match(/\['[^\]]*'\]/g) || []).length >= 30 && /HIT RATE BY LEAGUE \(athlete found \/ stats filled \/ Instagram handle \/ follower count\)/.test(s) && /stats \$\{t\.stats\} of/.test(s) && /instagram handle \$\{t\.handle\} of/.test(s); })());
