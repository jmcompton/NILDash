'use strict';
// No database, no network. A local HTTP server stands in for DeepSeek and
// answers by the person's name, so each filter reason is exercised once; a
// second stands in for the search provider; mail is sent nowhere (--no-email).
//
...
// What the stand-in DeepSeek answers, by name. Dan's call fails outright
// (a 401, which is not retried). The search provider answers every query
// with one result.
const BY_NAME = {
...
function startFakes() {
  const ds = http.createServer((req, res) => {
    let body = ''; req.on('data', (d) => { body += d; }); req.on('end', () => {
      ...
      const name = (String((user && user.content) || '').match(/PERSON: ([^,]+),/) || [])[1] || '';
      if (name === 'Dan Fox') { res.writeHead(401, { 'content-type': 'application/json' }); return res.end(JSON.stringify({ error: { message: 'boom' } })); }
      const hasToolResult = (j.messages || []).some((m) => m.role === 'tool');
      const message = Array.isArray(j.tools) && !hasToolResult
        ? { role: 'assistant', content: '', tool_calls: [{ id: 'c1', type: 'function', function: { name: 'web_search', arguments: JSON.stringify({ query: name + ' linkedin' }) } }] }
        : { role: 'assistant', content: JSON.stringify(BY_NAME[name] || { summary: 'unknown ' + name }) };
      res.writeHead(200, { 'content-type': 'application/json' });
      res.end(JSON.stringify({ model: j.model, choices: [{ message, finish_reason: 'stop' }], usage: { prompt_tokens: 300, completion_tokens: 60 } }));
    });
  });
  const sp = http.createServer((req, res) => { res.writeHead(200, { 'content-type': 'application/json' }); res.end(JSON.stringify({ organic: [{ title: 'Profile', link: 'https://example.com/p', snippet: 'A profile.' }] })); });
  return Promise.all([ds, sp].map((s) => new Promise((r) => s.listen(0, '127.0.0.1', () => r(s))))).then(([a, b]) => { PORTS = { ds: a.address().port, sp: b.address().port, close: () => { a.close(); b.close(); } }; });
}
...
function setup(extra) {
  ...
  fs.writeFileSync(_tp.join(root, 'config.json'), JSON.stringify(Object.assign({
    to: 'me@x.com', resendApiKey: 're_test', deepseekApiKey: 'sk-ds-test-0000000000', deepseekBaseUrl: `http://127.0.0.1:${PORTS.ds}`, serperApiKey: 'serper-test',
    myAddresses: ['me@x.com'], aboutMe: 'me', prospectKeywords: ['sports', 'coach', 'marketing', 'owner', 'compliance'],
  }, extra || {})));
...
function run(home, args) {
  return new Promise((resolve) => {
    const c = spawn(process.execPath, [REPO + 'tools/briefs/prospecting.js', ...args], { env: { ...process.env, HOME: home, USERPROFILE: home, BRIEFS_HOME: '', BRIEFS_MAIL_SOURCES: '', SERPER_SEARCH_URL: `http://127.0.0.1:${PORTS.sp}/search`, DEEPSEEK_API_KEY: '', SERPER_API_KEY: '', BRAVE_SEARCH_API_KEY: '', TAVILY_API_KEY: '' } });
...
async function main() {
  ...
  const a = setup();
  const outA = await run(a.home, ['--no-email']);
  ...
  ok('  a failed call is filtered and marked for retry', /- \*\*Dan Fox\*\*.*: research failed: DeepSeek HTTP 401.*\(will be retried next run\)/.test(briefA));
  ...
  ok('  the next run has only Dan left', /would research 1\./.test(await run(a.home, ["--dry"])));

  // ── 2. --exclude ─────────────────────────────────────────────────────────
  OUT.push('', '-- --exclude --');
  const b = setup();
  const outB = await run(b.home, ['--no-email', '--debug', '--exclude', 'Ann Lee,linkedin.com/in/caradiaz,GUSOTT', '--exclude=Nobody Real']);
  ...
  ok('  so they are back in the queue next run', /would research 4\./.test(await run(b.home, ["--dry"])));
  const c = setup();
  const outC = await run(c.home, ['--dry', '--exclude=https://www.linkedin.com/in/bobray/']);
