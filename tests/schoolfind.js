...
(async () => {
  ...
  ok('the search loop hands back the results it saw, so a web answer can be checked against its source; and the queries it issued and the stop reason, so an empty search and a cut-off answer are not both "found nothing"', /results\.push\(\{ query/.test(src('server/services/webSearchTool.js')) && /rounds, results, queries, finishReason \}/.test(src('server/services/webSearchTool.js')));
